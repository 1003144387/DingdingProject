...
Page({
 ...
 /**
  ...
  getPartsList() {
    const _this = this;
    dd.httpRequest({
      url: globalData.baseUrl + "/api/produceParts/stockOutWaittingDefend/list",
      headers:{
      ...
      success: function (res) {
        if (res.statusCode === 200) {
          ...
          if (i === param.filePath.length - 1) {
            ...
            _this.producePartsDefend(options);
          } else {
            i++;
            param["i"] = i;
   ...
   * 生产物品三防处理审批提交
   */
  producePartsDefend(param) {
    const _this = this;
    let url = globalData.baseUrl + "/api/producePartsDefend/add";
    dd.httpRequest({
      url: url,
      method: 'POST',
      data: {
