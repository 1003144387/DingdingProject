...
Page({
 ...
 /**
  ...
  /**
   * 获取库存中待软件灌装和设备校准的部件列表
   */
  getPartsList() {
    const _this = this;
    dd.httpRequest({
      url: globalData.baseUrl + "/api/produceParts/stockOutUnSoftInstallParts/list",
      headers:{
  ...
  onSubmitTap(event) {
    ...
    if (imagePathList.length > 0) {
      ...
      //无图片直接上传
      this.ProducePartsSoftInstall(param)
    }
  },
  /**
   * 批量上传图片
      ...
      success: function (res) {
        if (res.statusCode === 200) {
          ...
          if (i === param.filePath.length - 1) {
            ...
            _this.ProducePartsSoftInstall(options);
          } else {
            i++;
            param["i"] = i;
   ...
   * 生产物品软件灌装和设备校准审批提交
   */
  ProducePartsSoftInstall(param) {
    const _this = this;
    let url = globalData.baseUrl + "/api/producePartsSoftInstallApprove/add";
    dd.httpRequest({
      url: url,
      method: 'POST',
      data: {
