...
Page({
 ...
 /**
  ...
  getComponentList() {
    const _this = this;
    dd.httpRequest({
      url: globalData.baseUrl + "/api/produceComponent/unProduceApproce/list",
      headers:{
  ...
  onSubmitTap(event) {
    ...
    if (imagePathList.length > 0) {
      ...
      //无图片直接上传
      this.ComponentProduceApprove(param)
    }
  },
  /**
   * 批量上传图片
      ...
      success: function (res) {
        if (res.statusCode === 200) {
          ...
          if (i === param.filePath.length - 1) {
            ...
            _this.ComponentProduceApprove(options);
          } else {
            i++;
            param["i"] = i;
   ...
   * 组件装配审批提交
   */
  ComponentProduceApprove(param) {
    const _this = this;
    let url = globalData.baseUrl + "/api/produceComponent/produceApprove";
    dd.httpRequest({
      url: url,
      method: 'POST',
      data: {
